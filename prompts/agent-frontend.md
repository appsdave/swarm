...
1. **Commit and push all your code**:
   ...
   git add -A
   git commit -m "feat(frontend): <short summary of what you built>"
...
2. **Create a Pull Request to merge into main**:
   ```
   cd worktree-frontend
   gh pr create --base main --head agent/frontend --title "Frontend: <task summary>" --body "Auto-generated by swarm agent frontend. <brief description of changes>"
   ```

3. **Then update Redis status**:
   ```
   SET agent:frontend:status done
   SET agent:frontend:task "Frontend complete — PR created"
   ```

## Work Rules
- Stay inside `worktree-frontend/` — never modify files outside it
- Commit frequently to the `agent/frontend` branch and push when you reach milestones
