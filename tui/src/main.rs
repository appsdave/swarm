...
=======
    let tab_help = match app.active_tab {
        ActiveTab::Swarm => {
            ...
            format!("{launch_msg} | [Backspace] Edit")
        }
        ActiveTab::History => "[↑/↓] Scroll".into(),
        ActiveTab::Notifications => "[↑/↓] Scroll".into(),
    };
    ...
    let redis = app
        ...
        .unwrap_or_default();
    let text = format!(
        " [Tab] Switch | [q] Quit | {tab_help}{redis}"
    );
>>>>>>> Stashed changes
